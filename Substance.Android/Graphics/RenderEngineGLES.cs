...
{
    private const string c_platform = "Android";
    private const string c_api = "OpenGLES";
    ...
    internal RenderEngineGLES() : base(GraphicApi.OpenGL)
    {
        ...
        Log.Info($"[{nameof(RenderEngineGLES)}] 创建成功");
    ...
    protected unsafe override void DrawTextureOverride(uint texture, in Matrix3x2 transform, in Vector3<float> modulate)
    {
        ...
        {
            Log.Warning($"[{nameof(RenderEngineGLES)}] 未加载纹理 {texture}");
    ...
    protected override void OnDisposeOverride()
    {
        ...
        Log.Info($"[{nameof(RenderEngineGLES)}] 销毁成功");
    }
